    ...
    <div className="flex-1 flex items-center justify-center py-12">
      <div className="w-full max-w-lg">
        ...
        <div className="bg-card rounded-lg border border-border p-8 text-center">
          ...
          >
            {selectedAccount ? "Reconnect Wallet" : "Connect MetaMask Wallet"}
          </button>

          {selectedAccount && (
            <div className="mt-4 rounded-md border border-success/30 bg-success/10 px-4 py-3 text-left">
              <p className="text-xs text-success font-semibold">Wallet Connected</p>
              <p className="text-sm font-mono text-foreground mt-1 break-all">{selectedAccount}</p>
              <p className="text-xs text-muted-foreground mt-1">Visible as {truncatedAccount} in the top header.</p>
            </div>
          )}
