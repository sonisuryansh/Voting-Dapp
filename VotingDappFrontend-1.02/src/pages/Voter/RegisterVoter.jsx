...
const RegisterVoter = () => {
  ...
  let { contractInstance } = web3State;
  ...
  const handleVoterRegistration = async (e) => {
    ...
    try {
      ...
      // Ensure contract instance is available; prompt user to connect if not
      if (!contractInstance && handleWallet) {
        const newState = await handleWallet();
        contractInstance = newState?.contractInstance;
      }

      if (!contractInstance) {
        throw new Error("Wallet not connected or contract instance unavailable. Please connect MetaMask.");
      }

      // Map gender string to enum index expected by the contract
      const genderMap = { Male: 0, Female: 1, Other: 2 };
      const genderIndex = genderMap[gender] ?? 0;

      // Call the smart contract registerVoter method
      const tx = await contractInstance.registerVoter(name, Number(age), genderIndex);
      console.log("Transaction submitted:", tx);
      await tx.wait();
      alert("Voter registered successfully on-chain.");
    } catch (error) {
      console.error(error);
